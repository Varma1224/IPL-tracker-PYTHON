...
# Add an image
st.image("ipl_logo.png", use_column_width=True)  # You need to have the image file named 'ipl_logo.png' in the same directory as your script
...
if selected_season and option:
    ...
    # Which team has won the most matches in the selected season?
        ...
        st.markdown(f"<p style='font-size: 20px; font-weight: bold;'>Team with Most Wins in {selected_season}:</p>", unsafe_allow_html=True)
        if data is not None and 'Team' in data.columns:
            most_wins_team = data.groupby('Team')['Points'].sum().idxmax()
            st.markdown(f"<p style='font-size: 20px;'>{most_wins_team}</p>", unsafe_allow_html=True)
        else:
            st.write("Data does not contain necessary information.")
    ...
    # Which player scored the most runs in the selected season (Orange Cap)?
        ...
        st.markdown(f"<p style='font-size: 20px; font-weight: bold;'>Orange Cap Player (Highest Runs Scorer) in {selected_season}:</p>", unsafe_allow_html=True)
        ...
        if not orange_cap_data.empty:
            ...
            st.markdown(f"<p style='font-size: 20px;'>{orange_cap_player}</p>", unsafe_allow_html=True)
        else:
            st.markdown("<p style='font-size: 20px;'>No data available for the selected season.</p>", unsafe_allow_html=True)


    # Which player has taken the highest number of wickets in the selected season (Purple Cap)?
    elif option == 'Purple Cap Player':
        st.markdown(f"<p style='font-size: 20px; font-weight: bold;'>Purple Cap Player (Highest Wickets Taken) in {selected_season}:</p>", unsafe_allow_html=True)
        if 'Purple Cap Player' in orange_purple_data.columns:
            ...
            st.markdown(f"<p style='font-size: 20px;'>{purple_cap_player}</p>", unsafe_allow_html=True)
        else:
            st.markdown("<p style='font-size: 20px;'>Data does not contain necessary information.</p>", unsafe_allow_html=True)
